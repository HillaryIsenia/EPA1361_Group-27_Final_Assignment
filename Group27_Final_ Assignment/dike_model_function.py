...
class DikeNetwork(object):
    def __init__(self):
        ...
        #Group27 min and max waterlevel have been made attributes of the model so they become easy to call upon
        self.min_waterlevel = 100
        self.max_waterlevel = 0
...
#        ema_logging.info('model initialized')
    ...
    def __call__(self, timestep=1, **kwargs):
        ...
        for s in self.planning_steps:
                ...
                for t in range(1, len(time)):
                    ...
                    for n in range(0, len(dikelist)):
                        ...
                        if node['type'] == 'dike':
                            ...
                            # name variable for water level
                            #Group27 here the current waterlevel of the model, will be compared to 
                            # the former min/max waterlevel, to see if a new min/max has occured
                            waterlevel = node['wl'][t]
                            self.min_waterlevel = min(self.min_waterlevel, waterlevel)
                            self.max_waterlevel = max(self.max_waterlevel, waterlevel)
            ...
            #Group27 here the waterlevels are updated to the data dictionary to be able to call upon their values
            # in the problem_formulation.py
            data.update({'Min waterlevel {}'.format(s): self.min_waterlevel})
            data.update({'Max waterlevel {}'.format(s): self.max_waterlevel})
            data.update({'Max waterlevel': self.max_waterlevel})
